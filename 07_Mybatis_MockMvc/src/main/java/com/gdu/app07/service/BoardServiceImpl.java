...
public class BoardServiceImpl implements BoardService {
	...
	public List<BoardDTO> getBoardList() {
		return boardDAO.selectBoardList();	// DAO의 ArrayList를 받는다 -> 다시 controller로 돌려준다.
	}

	@Override
	public BoardDTO getBoardByNo(HttpServletRequest request) {
		// 파라미터 boardNo가 없으면(null, "") 0을 사용한다.	// Optional은 null 처리할 때만 사용한다.
		String strBoardNo = request.getParameter("boardNo");	// 파라미터는 string이여야 한다.
		int boardNo = 0;
